...
class SitemapController extends Controller
{
    ...
    public function create($website){
        ...
        $sitemap_data = [
            [
                ...
                'url' => $this->website . 'receive-sms-online/gat-phone-number.html',
                'changefreq' => 'daily',
                'score' => '0.9'
            ],[
                'url' => $this->website . 'receive-sms-online/foreign-phone-number.html',
                'changefreq' => 'daily',
                'score' => '0.9'
            ]
        ];
        ...
        $sitemap_data = $this->insertData($sitemap_data, $this->generateUrlNumber($this->website . 'receive-sms-online/phone-number/page/[number].html', 56));
        $sitemap_data = $this->insertData($sitemap_data, $this->generateUrlNumber($this->website . 'receive-sms-online/china-phone-number/page/[number].html', 40));
        $sitemap_data = $this->insertData($sitemap_data, $this->generateUrlNumber($this->website . 'receive-sms-online/uk-phone-number/page/[number].html', 5));
        $sitemap_data = $this->insertData($sitemap_data, $this->generateUrlNumber($this->website . 'receive-sms-online/usa-phone-number/page/[number].html', 8));
        $sitemap_data = $this->insertData($sitemap_data, $this->generateUrlNumber($this->website . 'receive-sms-online/canada-phone-number/page/[number].html', 2));
    ...
    public function generateCountryPhoneNumber(){
        ...
        for ($i = 0; $i < count($country_all_info); $i++){
            $country_phone_number[$i]['url'] = $this->website . 'receive-sms-online/' . strtolower($country_all_info[$i]['en_title']).'-phone-number.html';
    ...
    public function generatePhonePage(){
        ...
        for ($i = 0; $i < count($phone_info); $i++){
            $url = $this->website . 'receive-sms-online/' . strtolower($phone_info[$i]['country']['en_title']).'-phone-number/verification-code-'.$phone_info[$i]['phone_num'].'.html';
            $data[$i]['url'] = $url;
    ...
    public function generateArticle($id = []){
        ...
        for ($i = 1; $i < count($id) + 1; $i++){
            $data[$i]['url'] = $this->website . 'receive-sms-online/' . 'article/'.$id[$i-1].'.html';
            $data[$i]['changefreq'] = 'daily';
            $data[$i]['score'] = '0.9';
        }
        return $data;
    ...
    //根据网址,数量生成地址 https://sms.com/phone-number/page/[number].html
