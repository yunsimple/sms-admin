...
class GatController extends BaseController
{
    public function index(){
        ...
        if ($phone_num) {
            ...
            $redis_value = $redis->redisCheck(Request::subDomain() . '_web_gat_' . $page);
            if($redis_value){
                $result = unserialize($redis_value) ;
            }else{
                $result = $phone_model->getPartPhoneNum('gat');
                (new RedisController())->redisSetCache(Request::subDomain() . '_web_gat_' . $page, serialize($result));
            }
            //(new ClickController())->click(1, 'gat');
        }
...
//        $this->assign('count', $count);
        ...
        $this->assign('title', '中国香港澳门台湾-免费短信验证码在线接收平台');
